...
uint64_t cfgs=0,maskhits=0,safe_regions=0;int maximum_safe_run=0;
...
void consider(const Task&t,const string&p,const vector<bool>&safe,const string&name){
 cfgs++;int definite=count(safe.begin(),safe.end(),true),longest=0,runlen=0;
 for(int i=0;i<t.n;i++){if(safe[i]&&printable((unsigned char)p[i]))longest=max(longest,++runlen);else runlen=0;}
 maximum_safe_run=max(maximum_safe_run,longest);if(longest>=32)safe_regions++;
 bool ascii=definite>=80&&spanvalid(p,safe,0),utf=definite>=80&&spanvalid(p,safe,1);
 if(!ascii&&!utf&&longest<32)return;
 string out;for(int i=0;i<t.n;i++)out+=safe[i]?hex(p.substr(i,1)):"??";
 if(ascii||utf){maskhits++;cout<<"MASK_CANDIDATE label="<<t.label<<" algorithm="<<name<<" definite="<<definite<<" ascii="<<ascii<<" typography="<<utf<<" safe_run="<<longest<<" plaintext_mask="<<out<<endl;}
 else cout<<"SAFE_RUN label="<<t.label<<" algorithm="<<name<<" definite="<<definite<<" safe_run="<<longest<<" plaintext_mask="<<out<<endl;
...
int main(){
 ...
 cerr<<"DONE masked_modes inputs="<<inputs<<" configurations="<<cfgs<<" candidates="<<maskhits<<" safe_regions="<<safe_regions<<" maximum_safe_run="<<maximum_safe_run<<" cached_streams="<<kstreams.size()<<" seconds="<<chrono::duration<double>(chrono::steady_clock::now()-began).count()<<endl;
}
